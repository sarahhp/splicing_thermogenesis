...
```{r}
here::i_am("R/10_leafcutter_custom_diffsplice.Rmd")
library(here)
...
meta=read.table(here("sample_info/beige_splicing_groups.txt"), header=F, stringsAsFactors = F)
...
confounders=model.matrix( ~0+confounders )
confounders
```
...
summary(combo$p.adjust < 0.05 & abs(combo$deltapsi)> 0.1)#with a delta psi threshold though, we have less 
...
```{r}
summary(unique(lc_genes$genes) %in% all_sig$gene_name)
