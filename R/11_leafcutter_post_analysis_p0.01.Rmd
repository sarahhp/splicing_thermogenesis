...
library(enrichplot)
here::i_am("R/11_leafcutter_post_analysis_p0.01.Rmd")
library(here)
```
...
```{r}
go = enricher(unique(lc_genes$genes[lc_genes$p.adjust < 0.01 & abs(lc_genes$deltapsi) > 0.1]),
...
```{r}
sep_go = list()
gl = unique(lc_genes$genes[lc_genes$p.adjust < 0.01 & abs(lc_genes$deltapsi) > 0.1])
universe = unique(lc_genes$genes)
for (db in prefixes){
    print(db)
...
from_rosi = some.molsig[grep("HALLMARK_FATTY_ACID_METABOLISM|HALLMARK_ADIPOGENESIS|GOBP_MONOCARBOXYLIC_ACID_METABOLIC_PROCESS|GOCC_MITOCHONDRIAL_MATRIX|REACTOME_MITOCHONDRIAL_BIOGENESIS|GOBP_MITOCHONDRIAL_RESPIRATORY_CHAIN_COMPLEX_ASSEMBLY", some.molsig$term),]

specific = enricher(unique(lc_genes$genes[lc_genes$p.adjust < 0.01 & abs(lc_genes$deltapsi) >= 0.1]),
               universe = unique(lc_genes$genes),
