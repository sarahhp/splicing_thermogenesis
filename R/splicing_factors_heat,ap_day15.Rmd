...
library(here)
here::i_am("R/splicing_factors_heat,ap_day15.Rmd")
```
...
colnames(sig)
all_sig = sig$gene_name[rowSums(sig[startsWith(colnames(sig), "adj.P.Val.d")] < 0.05) == 6]
length(all_sig)
  
any_sig = sig$gene_name[sig$adj.P.Val < 0.01] 
length(any_sig)
```
...
summary(reg_rnasplice %in% any_sig) #only 43 are significant in any donor
43/157 #only 30% of expressed factors are DE in any donor
```

```{r}
summary(spliceaid %in% all_sig) #NONE are sig in all donors
summary(spliceaid %in% any_sig) #only 14 are significant in any donor
```
...
```{r}
hm = Heatmap(norm[rownames(norm) %in% c(reg_rnasplice, "UCP1") &
               rownames(norm) %in% any_sig,], height=5)
hm

pdf(here("R/plots", "reg_rnasplice_factors_any_sig.pdf"), width=5, height=7)
hm
dev.off
