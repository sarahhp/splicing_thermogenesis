...
ggsave(file.path(WINDOWS, histone, paste0(histone, "_violin_plot.pdf")))
...
## Normalise to background average and standard dev
And if we normalise explicitly to the not_sig TSSes; we get the same result.  Beige TSSes are enriched in H3K4me3 in beige adipocytes.  

```{r}
backgr = filter(long, tss_set == "not_sig" & !is.na(mean_enrichment))
backgr_sum = group_by(backgr, context) %>% summarise(median=median(mean_enrichment), stdev = sd(mean_enrichment))
backgr_sum
```


```{r}
long_bg = merge(long, backgr_sum, by="context")
long_bg = mutate(long_bg, relative_enrichment = (mean_enrichment-median)/stdev)
ggplot(long_bg,
       aes(x=tss_set, y=relative_enrichment, color=context)) + geom_violin() +
    geom_boxplot(fill=NA, width=0.5, position= position_dodge(0.9))  +
...
```{r}
compare_means(relative_enrichment ~ context, data=long_bg, method="anova", group.by="tss_set",)
summary(aov(relative_enrichment ~tss_set*context, data=long_bg))
```
