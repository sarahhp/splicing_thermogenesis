...
GO and volcano plots 
```{r message=FALSE, warning=FALSE}
library(EnhancedVolcano, lib.loc=.libPaths()[-1])
library(tidyr)
library(clusterProfiler, lib.loc=.libPaths()[-1])
here::i_am("R/leafcutter_post_analysis.Rmd")
library(here)
```
...
Modifying plot for publication. Using raster for the points to make the file size smaller.  
```{r}
EnhancedVolcano(leaf, lab=leaf$genes, x="deltapsi", y="p", 
                xlim=c(-0.7, 0.7), legendPosition = "right",
                pCutoffCol = "p.adjust", pCutoff = 0.01,
...
EnhancedVolcano(leaf, lab=leaf$genes, x="deltapsi", y="p", 
                xlim=c(-0.7, 0.7), legendPosition = "right",
                pCutoffCol = "p.adjust", pCutoff = 0.01,
                FCcutoff = 0.05, ylab = "-log10 P value per cluster",
                xlab="dPSI per junction",  legendLabels = c("Not Sig", "dPSI", "p-value", "p-value and dPSI"))
```
## Exon switching events
not for looking at on this dataset -> try leafviz output.  

```{r}
sig = filter(leaf, p.adjust < 0.05 & abs(deltapsi) > 0.1)
head(table(sig$genes))
```
...
```{r}
molsig <- clusterProfiler::read.gmt("C:/Users/sarahhp/OneDrive - Universitetet i Oslo/OUS/Data/annotations/msigdb_v2023.1.Hs_GMTs/msigdb.v2023.1.Hs.symbols.gmt")
...
```{r}
to_print = go[,c("ID","GeneRatio", "BgRatio"  ,
                  "pvalue"    ,  "p.adjust" ,   "qvalue"    ,  "geneID"   ,   "Count" )]
write.table(to_print, here("31_leafcutter","General_GO_ORA.txt"))
```


using 5% deltapsi (bit more lenient)
```{r}
go = enricher(unique(lc_genes$genes[lc_genes$p.adjust < 0.05 & abs(lc_genes$deltapsi) >= 0.05]),
               universe = unique(lc_genes$genes),
            TERM2GENE = some.molsig,
            pvalueCutoff = 1, qvalueCutoff = 0.9)
head(go)[3:9]
...
```{r}
sep_go = list()
gl = unique(lc_genes$genes[lc_genes$p.adjust < 0.05 & abs(lc_genes$deltapsi) >= 0.05])
universe = unique(lc_genes$genes)
for (db in prefixes){
    print(db)
    ...
    print(head(df[1:8], n=10))
    sep_go[[db]] = ea
    #print(dotplot(ea, showCategory=20) +ggtitle(db))
    #print(cnetplot(ea, geneSetID = 1:5) + ggtitle(db))
}

sig = dplyr::bind_rows(sep_go[["GOBP"]]@result, sep_go[["GOMF"]]@result, sep_go[["HALLMARK"]]@result, sep_go[["REACTOME"]]@result)
sig = sig[order(sig$p.adjust),]
head(sig)
sig_ob = multienrichjam::enrichDF2enrichResult(sig)
dotplot(sig_ob, color="qvalue", showCategory = 10)
...
specific = enricher(unique(lc_genes$genes[lc_genes$p.adjust < 0.05 & abs(lc_genes$deltapsi) >= 0.1]),
               universe = unique(lc_genes$genes),
            TERM2GENE = rbind(grep_sets, from_rosi),
            pvalueCutoff = 1, qvalueCutoff = 0.9)
head(specific, n=20)[3:9]
```

Limiting our scope adipogenesis with 9 genes and FA metabolism with 7 terms are just significant.  
