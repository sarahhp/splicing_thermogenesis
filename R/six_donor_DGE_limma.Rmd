...
library(limma)
library(edgeR)
library(biomaRt,lib.loc=.libPaths()[-1])
library(ggplot2)
library(ggrepel)
...
summary(anytable$adj.P.Val < 0.01) #much more conserative than previous tries
```
...
for (d in donors){
  ...
  alltab = alltab[!duplicated(alltab$gene_name),] #remove duplicates because they tend to propogate
}
