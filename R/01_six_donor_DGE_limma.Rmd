...
Figure 1E: GO of beige upregulated genes (adjusted p < 0.05) in all donors
 - R/plots/six_donor_DGE_limma/beige_go-1.pdf
...
annot = getBM(attributes=c("external_gene_name", "description", "gene_biotype","ensembl_gene_id", "ensembl_gene_id_version"), 
...
coi = makeContrasts(background = frac_assigned_to_genes,
                    s1 = b.s1 - w.s1,
                    s2 = b.s2 - w.s2,
                    s3 = b.s3 - w.s3,
                    s4 = b.s4 - w.s4,
...
vennDiagram(results[,c(4:7)])
vennDiagram(results[,c(2:3,6:7)])
```
...
```{r}
results_bin = make_upset_table(results[,2:7])
...
### Overall pvalue
We can extract a global pvalue across tests like this
...
ggplot(long[long$gene_name == "PPARG",]) + geom_bar(stat="summary",fun="mean", aes(x=donor.condition, y=fpkm, fill=condition)) + 
  geom_point(aes(x=donor.condition, y=fpkm)) + theme(axis.text.x=element_text(angle=45, hjust=1))
```
